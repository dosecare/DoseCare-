 ...
 * Keeps sachet regimens out of mg/mL concentration math.
 */
(function(global){
  'use strict';
  ...
  engine.calculate=function(args){
    ...
    if(r.type==='sachet_age_based'){
      const low=n(r.doseMin),high=n(r.doseMax);if(low===null||high===null||low<0||high<low)return {ok:false,error:'The configured sachet dose cannot be calculated safely.',code:'INVALID_DOSE'};
      return {ok:true,medicineId:args.medicine.id,medicine:args.medicine,regimen:r,formulation:f,weight:null,age:n(args.age),ageUnit:args.ageUnit,frequency:n(r.frequency)||1,frequencyText:r.frequencyText||null,lowSachets:low,highSachets:high,dailyLowSachets:low,dailyHighSachets:high,maximumSachetsPerDay:n(r.normalMaximum),calculationType:'sachet_age_based',concentrationText:f.display||null};
    }
    if(r.type==='sachet_schedule'){
      const schedule=(r.schedule||[]).map((s,i)=>{const dose=n(s.doseSachets);return dose===null||dose<0?null:{index:i+1,dayRange:s.dayRange||`Day ${i+1}`,doseSachets:dose};}).filter(Boolean);
