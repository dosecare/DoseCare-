...
(function (global) {
  ...
  function concentrationToMgPerMl(formulation) {
    ...
    if (formulation.concentration && num(formulation.concentration.amount) > 0 && num(formulation.concentration.volume) > 0) {
      return num(formulation.concentration.amount) / num(formulation.concentration.volume);
    }
    return null;
  }
  ...
  function validateBounds(regimen, weight, age, ageUnit) {
    ...
    const weeks = ageWeeks(a, ageUnit), months = ageMonths(a, ageUnit), years = ageYears(a, ageUnit);
  ...
  function calculateLabelAgeBased({ medicine, regimen, weight, age, ageUnit, formulation }) {
    const a = num(age);
    if (a === null || a < 0 || !['months', 'years', 'weeks'].includes(ageUnit)) return fail('Enter a valid child age.', 'INVALID_AGE');
  ...
  function calculateLabelWeightAge({ medicine, regimen, weight, age, ageUnit, formulation }) {
    ...
    if (!Array.isArray(regimen.table)) return fail('The labeled dose table is not configured correctly.', 'INVALID_DOSE_TABLE');
  ...
  function calculateScheduled({ medicine, regimen, weight, age, ageUnit, formulation }) {
    ...
    return { ok: true, medicineId: medicine.id, regimen, weight: w, age: num(age), ageUnit, frequencyText: regimen.frequencyText, frequency: 1, lowMg: schedule[0].doseMg, highMg: schedule[0].doseMg, lowMl: schedule[0].doseMl, highMl: schedule[0].doseMl, dailyLowMg: schedule[0].doseMg, dailyHighMg: schedule[0].doseMg, mgPerMl, maximumApplied: null, calculationType: 'scheduled_weight_based', concentrationText: formulation?.display || null, schedule };
  }
  ...
  function calculate({ medicine, regimen, age, ageUnit, weight, formulation }) {
    ...
    // Accept the canonical frequency field and legacy aliases used by existing V2 records.
    const frequency = num(regimen.frequency ?? regimen.dosesPerDay ?? regimen.frequencyPerDay ?? (normalizedType === 'mg_per_kg_per_dose' ? 1 : 1));
    ...
    if (normalizedType === 'mg_per_kg_per_day') {
      ...
      lowMg = min; highMg = max; if (frequency) { dailyLowMg = lowMg * frequency; dailyHighMg = highMg * frequency; }
    } else return fail(`Unsupported dosing type: ${regimen.type || 'unknown'}.`, 'UNSUPPORTED_DOSING_TYPE');

    // Support all existing maximum-dose field names while the database is being normalized.
    ...
    if (maximumDaily !== null && dailyHighMg !== null && dailyHighMg > maximumDaily && frequency) {
      const cappedLowDaily = Math.min(dailyLowMg, maximumDaily), cappedHighDaily = Math.min(dailyHighMg, maximumDaily);
      lowMg = cappedLowDaily / frequency; highMg = cappedHighDaily / frequency; dailyLowMg = cappedLowDaily; dailyHighMg = cappedHighDaily; maximumApplied = maximumDaily;
    }

    if (maximumPerAdministration !== null && highMg > maximumPerAdministration) {
      highMg = maximumPerAdministration;
      if (lowMg > maximumPerAdministration) lowMg = maximumPerAdministration;
      dailyLowMg = frequency ? lowMg * frequency : dailyLowMg;
      dailyHighMg = frequency ? highMg * frequency : dailyHighMg;
      maximumApplied = maximumApplied ?? maximumPerAdministration;
    }

    const mgPerMl = concentrationToMgPerMl(formulation);
    const lowMl = mgPerMl ? lowMg / mgPerMl : null, highMl = mgPerMl ? highMg / mgPerMl : null;
    const alternativeFrequency = num(regimen.alternativeFrequency ?? regimen.alternativeDosesPerDay);

    return { ok: true, medicineId: medicine.id, regimen, weight: w, age: a, ageUnit, frequency, alternativeFrequency: alternativeFrequency || null, lowMg, highMg, dailyLowMg, dailyHighMg, lowMl, highMl, mgPerMl, maximumApplied, calculationType: normalizedType, concentrationText: formulation?.display || null };
  }
