...
(function () {
  ...
  function doseText(r){
    if(r.type==='fixed_dose'&&(r.doseMl!==undefined||r.minDoseMl!==undefined||r.maxDoseMl!==undefined||r.volumeMl!==undefined)){
      const a=r.minDoseMl??r.doseMl??r.volumeMl,b=r.maxDoseMl??r.doseMl??r.volumeMl;
      return String(a===b?a:(a+'–'+b))+' '+(r.doseUnit||'mL/dose');
    }
    if(r.type==='sachet_age_based')return `${r.doseMin??r.minDose??r.dose} ${r.doseUnit||'sachet/day'}`;
