...
const steps=[];
if(r.type==='label_weight_age_based') steps.push(`<div class="calc-step"><span>01</span><div><strong>Label dose selection</strong><p>The dose was selected from the configured pediatric weight/age table for <b>${fmt(d.weight)} kg</b>${d.weightLb!=null?` (${fmt(d.weightLb)} lb)`:''}.</p></div></div>`);
...
} else if(r.type==='fixed_dose'||r.type==='age_based'||r.type==='label_age_based') steps.push(`<div class="calc-step"><span>01</span><div><strong>Fixed/age-based dose</strong><p>The configured pediatric dose is <b>${esc(dose)}</b> according to the selected regimen.</p></div></div>`);
else steps.push(`<div class="calc-step"><span>01</span><div><strong>Dose calculation</strong><p>${d.weight!=null?`${fmt(d.weight)} kg × `:''}${fmt(r.minDose??r.dose)}${r.maxDose!=null?`–${fmt(r.maxDose)}`:''} mg/kg/dose = <b>${esc(dose)}</b>.</p></div></div>`);
