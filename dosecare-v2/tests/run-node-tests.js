/* DoseCare V2 — Node.js runner for regression tests + readiness + structural database audit + production smoke checks. */
'use strict';
const fs = require('fs');
...
const productionSmoke = require('./production-smoke.test.js');
...
if (!result.passed || !dextromethorphanResult.passed || !vitaminD3Result.passed || !folicAcidResult.passed || !fluconazoleResult.passed || !readinessResult.passed || !arithmeticResult.passed || !audit.passed || !productionSmoke.passed) {
  process.exitCode = 1;
  console.error(`\nDoseCare V2 QA FAILED — ${result.results.length} core regression tests + ${dextromethorphanResult.results.length} Dextromethorphan tests + ${vitaminD3Result.results.length} Vitamin D3 tests + ${folicAcidResult.results.length} Folic acid tests + ${fluconazoleResult.results.length} Fluconazole tests + ${readinessResult.results.length} calculator-readiness checks + ${arithmeticResult.results.length} arithmetic-integrity checks + production smoke checks; database structural audit ${audit.passed ? 'passed' : 'failed'}.`);
} else {
  console.log(`\nDoseCare V2 QA PASSED — ${result.results.length} core regression tests + ${dextromethorphanResult.results.length} Dextromethorphan tests + ${vitaminD3Result.results.length} Vitamin D3 tests + ${folicAcidResult.results.length} Folic acid tests + ${fluconazoleResult.results.length} Fluconazole tests + ${readinessResult.results.length} calculator-readiness checks + ${arithmeticResult.results.length} arithmetic-integrity checks + production smoke checks + database structural audit (${audit.medicineCount} active oral-liquid medicines).`);
}
