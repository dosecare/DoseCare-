window.DoseCareV2Audit = (() => {
  ...
  const validTypes = new Set(['mg_per_kg_per_day','mg_per_kg_per_dose','condition_based','fixed_dose','age_based','label_age_based','label_weight_age_based','scheduled']);
  ...
  for (const m of medicines) {
    ...
    for (const f of (m.formulations || [])) {
      if (!f.id || !f.concentration || !(Number(f.concentration.amount) > 0) || !(Number(f.concentration.volume) > 0)) errors.push(`${m.id}: invalid formulation ${f.id || '(no id)'}`);
    }
    if (!Array.isArray(m.regimens) || !m.regimens.length) errors.push(`${m.id}: no regimens`);
    const formulationIds = new Set((m.formulations || []).map(f => f.id));
    for (const r of (m.regimens || [])) {
      ...
      for (const fid of (r.allowedFormulations || [])) if (!formulationIds.has(fid)) errors.push(`${m.id}/${r.id}: unknown allowedFormulation ${fid}`);
    }
