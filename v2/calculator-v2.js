...
(function () {
  ...
  const getSelectedRegimen = medicine => {
    if (!medicine || medicine.regimenSelectionRequired) return null;
  ...
  function renderMedicines() {
    ...
    allMedicines.slice().sort((a, b) => a.name.localeCompare(b.name)).forEach(m => {
      const option = document.createElement('option'); option.value = m.id; option.textContent = m.name; medicineSelect.appendChild(option);
    });
  }
  function renderMedicine() {
    const medicine = getMedicine();
    conditionSelect.innerHTML = '<option value="">Select condition</option>'; concentrationSelect.innerHTML = '';
    message.textContent = ''; conditionField.hidden = true; concentrationField.hidden = true; calculateButton.disabled = false;
    if (!medicine) { recommendedDose.textContent = 'Select a treatment first'; return; }
    const options = medicine.conditionOptions;
    if (options.length > 1) {
      conditionField.hidden = false;
      options.forEach(item => { const option = document.createElement('option'); option.value = item.label; option.textContent = item.label; conditionSelect.appendChild(option); });
      recommendedDose.textContent = 'Select a condition to view the dose';
    } else if (medicine.regimenSelectionRequired) {
      recommendedDose.textContent = 'Multiple clinical regimens require an indication-specific selection.'; calculateButton.disabled = true;
    } else recommendedDose.textContent = formatDose(medicine.regimens[0]);
    const forms = medicine.formulations;
    if (forms.length > 1) concentrationField.hidden = false;
    forms.forEach((f, index) => { const option = document.createElement('option'); option.value = String(index); option.textContent = formulationLabel(f); concentrationSelect.appendChild(option); });
  }
  function renderCondition() {
    const medicine = getMedicine(); if (!medicine) return;
    const regimen = getSelectedRegimen(medicine); recommendedDose.textContent = regimen ? formatDose(regimen) : 'Select a condition to view the dose';
  }
  function calculate() {
    const medicine = getMedicine();
    ...
    const regimen = getSelectedRegimen(medicine); if (!regimen) throw new Error('Please select a condition.');
    const forms = medicine.formulations; const index = forms.length > 1 ? Number(concentrationSelect.value) : 0; const formulation = forms[index] || forms[0] || null;
    const result = globalThis.DoseCareDosingEngine.calculate({ regimen, medicine, weight: $('weight-value').value, ageValue: $('age-value').value, ageUnit: $('age-unit').value, formulation });
    sessionStorage.setItem('dosecareV2Result', JSON.stringify({ medicine: medicine.raw, normalizedMedicine: medicine, condition: medicine.conditionOptions.length > 1 ? conditionSelect.value : null, formulation, ...result }));
  ...
  form.addEventListener('submit', event => { event.preventDefault(); message.textContent = ''; try { calculate(); } catch (error) { message.textContent = error.message || 'Unable to calculate this dose.'; } });
  renderMedicines();
})();
